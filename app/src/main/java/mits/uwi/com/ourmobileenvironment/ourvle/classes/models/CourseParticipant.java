 ...
 */
public class CourseParticipant extends SugarRecord{
    ...
    public void save() {
        ...
        List<UserCourse> dbUserCourses;
        for (int i = 0; i < enrolledcourses.size(); i++) {
            mUserCourse = enrolledcourses.get(i);
            mUserCourse.setUserid(this.userid);
            dbUserCourses = UserCourse.find(UserCourse.class,
                    "userid = ? and courseid = ?", userid + "", mUserCourse.getCourseid() + "");
    ...
    /**
     * set the course id of this user
     *
     * @param courseid
     */
